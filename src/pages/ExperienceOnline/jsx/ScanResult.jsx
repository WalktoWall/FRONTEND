  ...
  useEffect(() => {
    const fetchScanHistory = async () => {
      try {
        ...
        const [historyResponse, wishlistResponse] = await Promise.all([
          /* =========================
                 상품 스캔 히스토리
              ========================= */

          fetch(`${API_BASE_URL}/api/products/qr/history`, {
            method: "GET",
        ...
        console.log("상품 스캔 히스토리 응답:", historyData);
        ...
        const historyList = Array.isArray(historyData.productList)
          ? historyData.productList
          : [];

        const wishlistList = Array.isArray(wishlistData.productList)
        ...
        const wishlistIds = new Set(
          wishlistList.map((item) => Number(item.productId)),
        );
        ...
        const uniqueHistoryList = Array.from(
          new Map(
            historyList.map((product) => [Number(product.productId), product]),
          ).values(),
        );

        console.log("중복 제거 후 스캔 상품:", uniqueHistoryList);

        /* =========================
             화면용 데이터 가공
          ========================= */

        const formattedProducts = uniqueHistoryList.map((product) => ({
        ...
        console.error("ScanResult API 오류:", error);

        setErrorMessage(
          error.message || "상품 스캔 기록을 불러오지 못했습니다.",
        );
      } finally {
        setIsLoading(false);
      }
    ...
    <div className="scan-result-page">
      ...
      <main className="scan-result-main">
        ...
        <section className="scan-result-title-section">
          <h1 className="scan-result-title">제품 기록하기</h1>
        ...
        <section className="scan-result-product-scroll">
          <div className="scan-result-list">
            {isLoading ? (
              <p className="scan-result-loading">스캔 기록을 불러오는 중...</p>
            ) : errorMessage ? (
              <p className="scan-result-error">{errorMessage}</p>
            ) : products.length === 0 ? (
              <p className="scan-result-empty">아직 스캔한 상품이 없습니다.</p>
            ) : (
              products.map((product) => (
                <div className="scan-result-product-card" key={product.id}>
                  <Tag
