...
function MyPage() {
  ...
  const [visitCardDetail, setVisitCardDetail] = useState(null);
  ...
  useEffect(() => {
    const fetchMyPageData = async () => {
      try {
        ...
        const [myPageResponse, storesResponse, wishlistResponse] =
          await Promise.all([
        ...
        if (!myPageResponse.ok) {
          const errorText = await myPageResponse.text();
          ...
          throw new Error(`마이페이지 조회 실패: ${myPageResponse.status}`);
        }
        ...
        if (!storesResponse.ok) {
          ...
          console.error("매장 API 오류:", storesResponse.status, errorText);

          throw new Error(`매장 조회 실패: ${storesResponse.status}`);
        }
        ...
        console.log("매장 API 응답:", storesData);

        console.log("위시리스트 API 응답:", wishlistData);

        /* =========================
             USER
          ========================= */

        setUserName(myPageData.userName || "");
        ...
        setStyleBoard(
          Array.isArray(myPageData.styleBoard) ? myPageData.styleBoard : [],
        );
        ...
        setWishlistItems(
          Array.isArray(wishlistData.productList)
        ...
        if (visitCardId) {
          const visitCardResponse = await fetch(
            `${API_BASE_URL}/visitcards/${visitCardId}`,
            {
              method: "GET",
              headers,
            },
          ...
          if (!visitCardResponse.ok) {
            ...
            const visitCardDetailData = await visitCardResponse.json();

            console.log("Visit Card 상세 API 응답:", visitCardDetailData);

            setVisitCardDetail(visitCardDetailData);
            ...
            if (visitCardDetailData.visitCardId) {
              localStorage.setItem(
                "visitCardId",
                String(visitCardDetailData.visitCardId),
              );
            }
          }
        }
      } catch (error) {
        console.error("MyPage API 오류:", error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const latestStyleBoard = styleBoard.length > 0 ? styleBoard[0] : null;

  /* =========================
     STORE ID
  ========================= */

  const storeId = latestStyleBoard?.storeId;
  ...
  */
  ...
  const visitDate = visitCardDetail?.visitTime
    ? formatVisitDate(visitCardDetail.visitTime)
  ...
  const boardingTime =
    ...
    formatVisitTime(visitCardDetail?.visitTime) ||
  ...
  const handleShare = async () => {
    ...
    const shareData = {
      title: "MCM Style Storyboard",

      text: `${
        userName ? `${userName}님의` : "나의"
    ...
    <div className="mypage-page">
      ...
      <main className="mypage-main">
        ...
        <section className="mypage-passport-card">
          <p className="mypage-passport-label">My passport</p>
        ...
        {apiError && <p className="mypage-api-error">{apiError}</p>}
        ...
        <section className="mypage-storyboard">
          <div className="mypage-storyboard-title">MCM Style Storyboard</div>
          ...
          <div className="mypage-ticket">
            ...
            <h3 className="mypage-ticket-store">
              {isLoading ? "매장 정보를 불러오는 중..." : selectedStore}
            </h3>
            ...
            <div className="mypage-ticket-info">
              <div>
                ...
                <strong>{isLoading ? "--:--" : boardingTime}</strong>
              </div>
            </div>
          </div>
          ...
          <div className="mypage-life-card">
            <span className="mypage-life-label">Life</span>
            ...
            <div className="mypage-life-route">
              ...
              <img src={airplaneWhite} alt="" className="mypage-life-plane" />
            ...
            <span className="mypage-life-label">MCM</span>
          </div>
        </section>
        ...
        >
          ...
          <img src={shareIcon} alt="" className="mypage-share-icon" />
        </button>
        ...
        <section className="mypage-wishlist-section">
          ...
          <div className="mypage-wishlist-list">
            {isLoading ? (
              <p className="mypage-wishlist-loading">불러오는 중...</p>
            ) : wishlistItems.length === 0 ? (
              <p className="mypage-wishlist-empty">위시리스트가 없습니다.</p>
            ) : (
              wishlistItems.slice(0, 2).map((item) => (
                <div className="mypage-wishlist-card" key={item.productId}>
                  <Tag
                    size={29}
                    strokeWidth={1.6}
                  ...
                  <img src={starIcon} alt="" className="mypage-wishlist-star" />
                </div>
              ))
            )}
          </div>
        ...
        <button
          ...
          onClick={() => {
            window.location.href = "https://kr.mcmworldwide.com/";
          }}
        >
          MCM 홈페이지 가기
        </button>
