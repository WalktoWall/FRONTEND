...
const STAFF_VISITS_API = `${API_BASE_URL}/api/staff/visits/today`;
...
function getCompletedVisitIds() {
  try {
    const savedData = window.sessionStorage.getItem(COMPLETED_VISITS_KEY);
    ...
    */
    ...
    return parsedData.map((id) => Number(id));
  } catch (error) {
    console.error("응대 완료 목록 조회 오류:", error);
  ...
  useEffect(() => {
    const fetchTodayVisits = async () => {
      try {
        ...
        console.log("오늘의 방문 고객 조회 시작");

        /* =========================
             API 요청
          ========================= */

        const response = await fetch(STAFF_VISITS_API, {
          method: "GET",
        ...
        if (!response.ok) {
          ...
          throw new Error(`오늘의 방문 고객 조회 실패 (${response.status})`);
        }
        ...
        console.log("오늘의 방문 고객 API 응답:", data);
        ...
        if (!Array.isArray(data)) {
          console.error("방문 고객 API 응답이 배열이 아닙니다:", data);

          throw new Error("방문 고객 응답 형식이 올바르지 않습니다.");
        }
        ...
        const sortedVisits = sortVisitsByTime(data);

        setVisits(sortedVisits);
      } catch (error) {
        console.error("오늘의 방문 고객 조회 오류:", error);
    ...
    >
      ...
      >
        ...
        {!isLoading &&
          !errorMessage &&
          visits.map((visit) => {
            const { storeName, userId, userName, visitCardId, visitTime } =
              visit;
            ...
            const isCompleted = completedVisitIds.includes(Number(visitCardId));
            ...
            const displayedStatus = isCompleted
              ? "응대 완료"
              : formattedTime
                ? `${formattedTime} 방문 예정`
                : "방문 예정";
              ...
              >
                ...
                <span
                  className="
                    ...
                    "
                >
                  <strong>{userName ? `${userName}님` : "고객님"}</strong>
