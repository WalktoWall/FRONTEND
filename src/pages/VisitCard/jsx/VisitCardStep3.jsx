...
const getStoreRegion = (store) => {
  const storeName = store.storeName ?? store.name ?? "";

  if (storeName.includes("대구")) {
    return { region: "대구", fullRegion: "대구광역시" };
  }

  if (storeName.includes("광주")) {
    return { region: "광주", fullRegion: "광주광역시" };
  }

  return REGION_CATEGORY[store.regionCategory];
};
...
function VisitCardStep3({
  ...
  const stores = useMemo(() => {
    const normalizedStores = storeSource.map((store) => {
      const regionInfo = getStoreRegion(store);
